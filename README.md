...
| 平台 | 状态 | 安装方式 | 说明 |
|------|------|---------|------|
| Windows 10/11 (x64) | 支持 | 可执行文件 / 本地安装 | 便携包提供 CPU 与 NVIDIA CUDA 两种；本地安装另支持 DirectML |
| Linux (Ubuntu/Debian) | 支持 | 可执行文件 / 本地安装 | 便携包提供 CPU 与 NVIDIA CUDA 两种；ROCm/XPU 需本地安装专用 PyTorch 栈 |
| WSL2 | 支持 | 本地安装 | CPU、NVIDIA CUDA；WebUI 默认地址为 `http://127.0.0.1:7860` |
| Google Colab | 支持 | Notebook | Notebook 创建独立 Python 3.10 CUDA 环境，并显式检查 CUDA Provider |
| Hugging Face Spaces | 支持 | Space | 默认 CPU；付费 GPU Space 需改用对应后端依赖 |
| macOS | 实验性 | 本地安装 | 安装器支持 MPS；默认分离模型组合仍缺少 Apple Silicon 真机完整翻唱验证 |
...
3. 为包内的 `AI-RVC-Linux` 添加执行权限
4. 运行包内的 `AI-RVC-Linux`
...
PolarFormer 的 ONNX Provider 是显式配置：CUDA 使用 `CUDAExecutionProvider`，DirectML 使用 `DmlExecutionProvider`；ROCm、XPU、MPS 使用 `CPUExecutionProvider`。后三种后端的 PyTorch 模型仍使用所选加速器。启动日志会显示实际 Provider，Provider 不匹配时直接报错，不会静默切换。
...
# 4. 安装与后端匹配的项目依赖（三选一）
pip install -r requirements_cpu.txt   # CPU；ROCm/XPU/MPS 的 PolarFormer 也使用此 ONNX 组合
...
`python install.py` 会按项目约束选择后端依赖。手动安装请使用 `requirements_cpu.txt`、`requirements_cuda.txt` 或 `requirements_dml.txt`；`requirements.txt` 只保存后端中立的公共依赖。当前依赖栈使用 Gradio 5 与 NumPy 2，并固定 `audio-separator==0.44.1`。
