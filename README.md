...
当前源码正在准备 **1.5.0-rc.1**，尚未发布新包或 Docker 仓库镜像。Docker 本地构建、持久化和登录用法见 [Docker 使用指南](docs/Docker使用指南.md)，发行核对见[发布准备](docs/发布准备.md)。
...
audio-separator 0.47.0 要求 Apple Silicon 使用 macOS 14+ 和 PyTorch 2.13–2.x。先安装系统的 ARM 音频库；`samplerate==0.1.0` 内置的旧 macOS 库不能作为 Apple Silicon 的运行库。
